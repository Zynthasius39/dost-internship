...
docker_verify_minimum_env() {
	if [ -z "$MYSQL_ROOT_PASSWORD" -a -z "$MYSQL_ALLOW_EMPTY_PASSWORD" -a -z "$MYSQL_RANDOM_ROOT_PASSWORD" -a -z "$MYSQL_ROOT_PASSWORD_HASH" ]; then
		mysql_error <<-'EOF'
...
docker_setup_env() {
	...
	file_env 'MYSQL_ROOT_PASSWORD_HASH'
...
docker_setup_db() {
	...
	if [ -n "$MYSQL_ROOT_HOST" ] && [ "$MYSQL_ROOT_HOST" != 'localhost' ]; then
		...
		if [ -n "$MYSQL_ROOT_PASSWORD_HASH" ]; then
			read -r -d '' rootCreate <<-EOSQL || true
				CREATE USER 'root'@'${MYSQL_ROOT_HOST}' IDENTIFIED WITH 'caching_sha2_password' AS ${MYSQL_ROOT_PASSWORD_HASH} ;
	...
	if [ -n "$MYSQL_ROOT_PASSWORD_HASH" ]; then
		read -r -d '' passwordSet <<-EOSQL || true
			ALTER USER 'root'@'localhost' IDENTIFIED WITH 'caching_sha2_password' AS ${MYSQL_ROOT_PASSWORD_HASH} ;
		EOSQL
	else
		read -r -d '' passwordSet <<-EOSQL || true
