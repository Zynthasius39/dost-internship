...
- [End-to-End Kubernetes Application Delivery and
  Observability](#end-to-end-kubernetes-application-delivery-and-observability)
  - [Task Addımları](#task-addımları)
    - [1. Sadə app deploy et
      (traefik/whoami)](#1-sadə-app-deploy-et-traefikwhoami)
    - [2. Deployment və Service
      yaz](#2-deployment-və-service-yaz)
      - [2.1. *Deployment*
        resursu](#21-deployment-resursu)
      - [2.2. *Service*
        resursu](#22-service-resursu)
      - [2.3. Təsdiq
        et](#23-təsdiq-et)
    - [3. Gateway API qur](#3-gateway-api-qur)
      - [3.1 *Gateway*
        resursu](#31-gateway-resursu)
      - [3.2. *HTTPRoute*
        resursu](#32-httproute-resursu)
      - [3.3. Təsdiq
        et](#33-təsdiq-et)
    - [4. Domain ilə expose et (e.g., /etc/hosts
      ilə)](#4-domain-ilə-expose-et-eg-etchosts-ilə)
      - [4.1. *DNS A record*
        resursu](#41-dns-a-record-resursu)
      - [4.2. Təsdiq et](#42-təsdiq-et)
    - [5. 2 replica ver](#5-2-replica-ver)
      - [5.1. Deployment-i scale
        et](#51-deployment-i-scale-et)
    - [6. Rolling update test
      et](#6-rolling-update-test-et)
      - [6.1. *Deployment*-dakı image versiyasını
        qaldır](#61-deployment-dakı-image-versiyasını-qaldır)
      - [6.2. Təsdiq et](#62-təsdiq-et)
    - [7. Prometheus və Grafana qur (Helm ilə
      olar)](#7-prometheus-və-grafana-qur-helm-ilə-olar)
      - [7.1. Helm chart-ı
        quraşdır](#71-helm-chart-ı-quraşdır)
      - [7.2. Təsdiq et](#72-təsdiq-et)
    - [8. Node metrics
      izlənilsin](#8-node-metrics-izlənilsin)
    - [9. Pod metrics
      izlənilsin](#9-pod-metrics-izlənilsin)
    - [10. Alert rule yaz (CPU \>
      80%)](#10-alert-rule-yaz-cpu--80)
      - [10.1. *PrometheusRule*
        resursu](#101-prometheusrule-resursu)
      - [10.2. Təsdiq et](#102-təsdiq-et)
    - [11. HPA qur (min: 1, max:
      5)](#11-hpa-qur-min-1-max-5)
    - [12. Application load test et (e.g., hey, ab, simple
      loop)](#12-application-load-test-et-eg-hey-ab-simple-loop)
      - [12.1. *Deployment*-in testdən əvvəlki vəziyyətinə
        bax](#121-deployment-in-testdən-əvvəlki-vəziyyətinə-bax)
      - [12.2. Load testini
        başlat](#122-load-testini-başlat)
      - [12.3. Test ərzində baş verən horizontal autoscaling-i müşahidə
        et](#123-test-ərzində-baş-verən-horizontal-autoscaling-i-müşahidə-et)
      - [12.4. Test bitdikdən sonra hey və Grafana-da nəticəyə
        bax](#124-test-bitdikdən-sonra-hey-və-grafana-da-nəticəyə-bax)
